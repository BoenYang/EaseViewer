...
public class Activity_Album extends Activity {
	...
	private void init() {
		...
		gridImageAdapter = new Adapter_AlbumGridView(this, dataList,
				selectedDataList);
	...
	private void initSelectImage() {
		...
		for (final String path : selectedDataList) {
			ImageView imageView = (ImageView) LayoutInflater.from(
					Activity_Album.this).inflate(R.layout.choose_imageview,
					selectedImageLayout, false);
			selectedImageLayout.addView(imageView);
			hashMap.put(path, imageView);
			ImageManager.from(Activity_Album.this).displayImage(imageView,
	...
	private void initListener() {
		...
		gridImageAdapter
				.setOnItemClickListener(new Adapter_AlbumGridView.OnItemClickListener() {
						...
						if (isChecked) {
							if (!hashMap.containsKey(path)) {
								ImageView imageView = (ImageView) LayoutInflater
										.from(Activity_Album.this).inflate(
								...
								ImageManager.from(Activity_Album.this)
										.displayImage(imageView, path,
												R.drawable.pic_add_btn, 100,
												100);
								imageView
	...
	private void refreshData() {
		...
		new AsyncTask<Void, Void, ArrayList<String>>() {
			...
			protected void onPostExecute(ArrayList<String> tmpList) {
				...
				if (Activity_Album.this == null
						|| Activity_Album.this.isFinishing()) {
					return;
				}
				progressBar.setVisibility(View.GONE);
				dataList.clear();
