...
public class Activity_Album extends Activity{
	...
	private void initSelectImage() {
		...
		for(final String path:selectedDataList){
			ImageView imageView = (ImageView) LayoutInflater.from(Activity_Album.this).inflate(R.layout.choose_imageview, selectedImageLayout,false);
			selectedImageLayout.addView(imageView);			
			hashMap.put(path, imageView);
			ImageManager.from(Activity_Album.this).displayImage(imageView, path,R.drawable.camera_default,100,100);
		...
		gridImageAdapter.setOnItemClickListener(new AlbumGridViewAdapter.OnItemClickListener() {
			...
			public void onItemClick(final ToggleButton toggleButton, int position, final String path,boolean isChecked) {
				...
				if(selectedDataList.size()>=8){
					...
					if(!removePath(path)){
						Toast.makeText(Activity_Album.this, "只能选择8张图片", 200).show();
					}
					return;
				}
    ...
    private void refreshData(){
    	...
    	new AsyncTask<Void, Void, ArrayList<String>>(){
			...
			protected void onPostExecute(ArrayList<String> tmpList) {
				...
				if(Activity_Album.this==null||Activity_Album.this.isFinishing()){
					return;
				}
				progressBar.setVisibility(View.GONE);
				dataList.clear();
